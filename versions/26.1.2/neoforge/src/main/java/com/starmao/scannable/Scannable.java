...
public final class Scannable {
    ...
    public Scannable(IEventBus modEventBus, ModContainer modContainer) {
        ...
        // --- Client-only setup ---
        if (FMLEnvironment.getDist().name().equals("CLIENT")) {
        ...
        modEventBus.addListener(DataGeneration::gatherServerData);
    }
    ...
    private static void registerCapabilities(RegisterCapabilitiesEvent event) {
        ...
        // Use raw type cast to bridge the deprecated EnergyStorage API with
        // the new EnergyHandler-based capability system.
        event.registerItem((net.neoforged.neoforge.capabilities.ItemCapability) (Object) Capabilities.Energy.ITEM,
                (stack, ctx) -> (Object) com.starmao.scannable.common.energy.neoforge.ScannerEnergyStorage.of(stack),
                Items.SCANNER.get());

        // Item handler — exposes the scanner's internal module inventory
        // to hoppers, other mods, etc.
        event.registerItem((net.neoforged.neoforge.capabilities.ItemCapability) (Object) Capabilities.Item.ITEM,
                (stack, ctx) -> (Object) new ScannerItemHandler(ScannerContainer.of(stack)),
                Items.SCANNER.get());
